...
def encode_task(obs_history: List[np.array], action_history: List[str], prompt):
    ...
    for i in range(len(action_history)):
        ...
        message_trajectory['content'].append(
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{encode_image_array(obs_history[i])}"}})
        message_trajectory['content'].append({"type": "text", "text": f'Action: {action_history[i]}\n'})
    message_trajectory['content'].append({"type": "text", "text": 'Observation:'})
    message_trajectory['content'].append(
        {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{encode_image_array(obs_history[-1])}"}})

    messages.append(message_trajectory)
    messages.append({"role": "user",
                     "content": [{"type": "text",
                                  "text": "Action(Note that your action MUST be the aforementioned 'action code'.): "}]})
    return messages


class GPT4VAgentClient(AgentClient):
    def __init__(self, prompt: str, api_key=OPENAI_API_KEY, base_url=OPENAI_BASE_URL) -> None:
        if api_key:
